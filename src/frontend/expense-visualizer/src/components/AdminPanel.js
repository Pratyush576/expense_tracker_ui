...
const AdminPanel = ({ currentUser }) => {
    ...
    const adminRoutes = [
        ...
        <Route key="index" index element={<h2>Welcome to the Admin Panel</h2>} />
            ...
            <Row>
                <Col md={3} lg={2} className="bg-light sidebar">
                    <Nav className="flex-column">
                        {isAdmin && <Nav.Link as={Link} to="users">User Management</Nav.Link>}
                        {isAdmin && <Nav.Link as={Link} to="pricing">Pricing</Nav.Link>}
                        {isAdmin && <Nav.Link as={Link} to="discounts">Discounts</Nav.Link>}
                        {isAdmin && <Nav.Link as={Link} to="proposals">Proposals</Nav.Link>}
                        
                        {(isAdmin || isManager) && <Nav.Link as={Link} to="my-proposals">My Proposals</Nav.Link>}
                        {(isAdmin || isManager) && <Nav.Link as={Link} to="new-proposal">New Proposal</Nav.Link>}
                    </Nav>
                </Col>
                <Col md={9} lg={10}>
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <div>
                            <Link to="/">
                                <Button variant="secondary" className="me-2">Back to User Page</Button>
                            </Link>
